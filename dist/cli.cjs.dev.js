...
var writeChangeset = _interopDefault(require('@changesets/write'));
...
var isCI$1 = _interopDefault(require('is-ci'));
...
async function infoAllow404(pkgName) {
  ...
  if (pkgInfo.error && pkgInfo.error.code === "E404") {
    logger.warn(`Received 404 for npm info ${chalk.cyan(`"${pkgName}"`)}`);
    return {
      published: false,
      pkgInfo: {}
    };
  ...
  if (pkgInfo.error) {
    logger.error(`Received an unknown error code: ${pkgInfo.error.code} for npm info ${chalk.cyan(`"${pkgName}"`)}`);
...
async function internalPublish(pkgName, opts, twoFactorState) {
  ...
  if (!response || response && response.error) {
    ...
    if ((response.error.code === "EOTP" || response.error.code === "E401" && response.error.detail.includes("--otp=<code>")) && !isCI) {
      if (twoFactorState.token !== null) {
    ...
    logger.error(`an error occurred while publishing ${pkgName}: ${response.error.code}`, response.error.summary, response.error.detail ? "\n" + response.error.detail : "");
    return {
      published: false
    };
  }
  ...
  for (const pkgInfo of results) {
    ...
    if (!publishedVersions.includes(localVersion)) {
      ...
      if (preState !== undefined && publishedState === "only-pre") {
        logger.info(`${name} is being published to ${chalk.cyan("latest")} rather than ${chalk.cyan(preState.tag)} because there has not been a regular release of it yet`);
      }
    } else {
...
function showNonLatestTagWarning(tag, preState) {
  ...
  if (preState) {
    logger.warn(`You are in prerelease mode so packages will be published to the ${chalk.cyan(preState.tag)}
...
run$1(input, flags, cwd).catch(err => {
  if (err instanceof errors.InternalError) {
    ...
    logger.error(`https://github.com/atlassian/changesets/issues/new?title=${encodeURIComponent(`Unexpected error during ${input[0] || "add"} command`)}&body=${encodeURIComponent(`## Error
